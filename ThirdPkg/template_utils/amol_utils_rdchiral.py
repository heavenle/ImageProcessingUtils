...
import ThirdPkg.template_utils.coley_utils_rdchiral as cc
# import rdc 
...
class Reaction:
    ...
    def __init__(self, rsmi, rid=None):
        ...
        """
        self.rsmi = rsmi #原子映射反应smiles
        self.rid = rid
        self.rsmi, self.clean_rsmi = self.clean_reaction()

        self.get_reactant_list() # 获取反应物列表
        self.get_agent_list() # 获取代理列表
        self.get_product_list() # 获取生成物列表
        self.get_reactant_mol_list() # 获取反应物mol列表
        self.get_product_mol_list() # 获取生成物mol列表
    ...
    def generate_reaction_template(self, radius=1):
        ...
        Parameters:
            ...
            半径（int）：半径是指离反应中心要提取的原子数（环境）即radius = 1（默认）返回反应中心周围的第一个邻居
            立体化学（bool）：如果为True，立体化学将保留在提取的模板中。如果为False（默认）立体化学不会被明确地考虑。
        ...
        # ==================================================
        # try to sanitize molecules 清洁数据
        # TODO: could merge with sanitize reaction
        # ==================================================
        try:
            # AllChem模块包含了许多高级化学计算和操作函数.
            # RemoveHs 函数用于从分子对象中移除所有的氢原子（Hydrogen atoms）。
            for i in range(len(reactants)):
                reactants[i] = AllChem.RemoveHs(reactants[i])  # *might* not be safe
            for i in range(len(products)):
                products[i] = AllChem.RemoveHs(products[i])  # *might* not be safe
            # SanitizeMol 是 RDKit 中用于检查和修正分子结构的有效性和一致性的函数。
            # 它确保分子对象符合化学规则，并且其内部表示是正确的，这对于后续的计算和分析非常重要。
            [Chem.SanitizeMol(mol) for mol in reactants + products]  # redundant w/ RemoveHs
            # mol.UpdatePropertyCache() 是一个非常重要的函数，用于更新分子对象的内部属性缓存。
            # 这个函数确保分子的所有属性（如原子的显式氢原子数、价键状态等）都是最新的，并且与当前的分子结构一致。
            [mol.UpdatePropertyCache() for mol in reactants + products]
        except Exception as e:
            # can't sanitize -> skip
            print(e)
    ...
    def assess_retro_template(self, template, reactant_mol_list, retro_outcomes):
        ...
        for outcome_set in retro_outcomes:
            if cc.USE_STEREOCHEMISTRY is False:
                outcome_set_inchi = [Chem.MolToInchi(Chem.MolFromSmiles(outcome.replace('@', ''))) for outcome in
                                     outcome_set.split('.')]
            else:
...
class Parsers:
    ...
    def import_USPTO(self, data):
        ...
        return df.apply(lambda row: pd.Series([row['rsmi'].split()[0],
                                               str(row['patent_id']) + ";" + str(row['para_num']) + ";" + str(
                                                   row['year']),
                                               float('nan'),
                                               row['classification']
                                               ],
